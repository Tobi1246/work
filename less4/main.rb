...
class Main
 ...
 # У пользователя есть меню , тут ему лузить не нужно 
 ...
 def create_route
  ...
  def look_trains_on_station
    ...
    exist_station?(name).train_list.each_with_index do |item, index|
      puts "#{index + 1}: поезд № #{item.number} тип #{item.type}, вагонов - #{item.wagon.size}"
    end
  end
